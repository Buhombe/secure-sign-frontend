...
const { authLimiter, mfaLimiter, refreshLimiter } = require('../middleware/rateLimiter');
...
router.post('/signup', authLimiter, validate('signup'), async (req, res) => {
  ...
  try {
    ...
    try {
      await enqueueVerificationEmail({ userId: user.id, recipientEmail: email, verifyLink });
    ...
    // DocuSign-style: do NOT issue tokens on signup.
    // Account is "Pending" until the email activation link is clicked.
    // No JWT is returned — user must activate before they can log in.
...
router.post('/resend-verification', authLimiter, validate('resendVerification'), async (req, res) => {
  ...
  try {
    ...
    try {
      await enqueueVerificationEmail({ userId: user.id, recipientEmail: user.email, verifyLink });
...
router.post('/forgot-password', authLimiter, validate('forgotPassword'), async (req, res) => {
  ...
  try {
    ...
    try {
      await enqueuePasswordResetEmail({ userId: user.id, recipientEmail: user.email, resetLink });
...
router.post('/login', authLimiter, validate('login'), async (req, res) => {
  ...
  try {
    ...
    // DocuSign-style: block ALL login until email is activated.
    // Return activation_required so frontend shows dedicated activation screen.
    if (!user.email_verified) {
...
router.post('/refresh', refreshLimiter, async (req, res) => {
  try {
    ...
    // CSRF ROTATION: Rotate CSRF token alongside JWT refresh token.
    // The new token is returned in the response body so the frontend
    // can update its in-memory value immediately without a second round-trip.
    // The signed cookie is also updated in the same Set-Cookie header.
    const newCsrfToken = csrfRotate(res);
...
router.post('/logout', async (req, res) => {
  try {
    ...
    // CSRF CLEANUP: Clear CSRF cookie on logout.
    // Prevents stale token from being usable after session ends.
...
router.post('/logout-all', authMiddleware, async (req, res) => {
  try {
    ...
    // CSRF CLEANUP: Clear CSRF cookie when all sessions are revoked.
    clearTokenCookie(res);
...
router.post('/mfa/authenticate', authMiddleware, mfaLimiter, validate('verifyMfa'), async (req, res) => {
  const { token } = req.body;
  try {
    const userResult = await pool.query(
