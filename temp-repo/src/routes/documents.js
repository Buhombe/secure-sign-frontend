...
// ─────────────────────────────────────────────────────────────────────────────
router.post('/upload', authMiddleware, requireEmailVerified, upload.single('pdf'), validate('uploadDocument'), async (req, res) => {
  try {
    ...
    if (recipient_email && recipientTokenRaw) {
      ...
      try {
        await sendSigningEmail(recipient_email, signingLink, req.file.originalname);
        emailSent = true;
      } catch (emailErr) {
...
router.get('/:id/stream', authMiddleware, validateParams('documentId'), async (req, res) => {
  try {
    ...
    const { streamToResponse } = require('../services/storageService');
    await streamToResponse(result.rows[0].file_path, res);
  } catch (err) {
