...
// Updated: handles new statuses (in_progress, completed, voided, expired)
// Backward compatible — still works if backend returns old 'signed'/'pending' statuses
...
export default function Dashboard() {
  ...
  // Counts — handle both old and new status values
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: 'clamp(1rem, 3vw, 2rem)', overflowX: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            {[
              { num: pending,    label: 'Pending',     sub: 'Awaiting action',           color: '#d97706' },
              { num: inProgress, label: 'In Progress', sub: 'Workflow active',            color: '#2563eb' },
              { num: completed,  label: 'Completed',   sub: 'Fully signed',               color: '#16a34a' },
            ...
            <div className="doc-table">
              <style>{`
                ...
                @media (max-width: 640px) {
                  ...
                  .doc-col-status, .doc-col-date { text-align: left; }
                ...
                return (
                  <div key={doc.id} className="doc-row" style={{ background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <div className="doc-col-status">
                      <span style={{ display: 'inline-block', padding: '0.25rem 0.7rem', borderRadius: 20, fontSize: '0.75rem', fontWeight: 600, background: st.bg, color: st.color }}>
                        {st.label}
                      </span>
                    </div>
