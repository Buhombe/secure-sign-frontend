...
import { useAuth } from '../context/AuthContext'; // FIX P1
...
export default function SignDocument() {
  const { getToken } = useAuth(); // FIX P1: token from memory not localStorage
  const { id } = useParams();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Store signed doc URL for download
      if (data.signed_file_url || data.file_url) {
        setSignedDocUrl(data.signed_file_url || data.file_url);
      }
      setDone(true);
    } catch (e) {
  ...
  const handleLegacySign = async (img) => {
    ...
    try {
      ...
      let response;
      if (effectiveToken) {
        response = await fetch(`${API_BASE}/signers/${id}/sign-public-legacy`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      let url = signedDocUrl;
      // If no URL from signing response, fetch from stream endpoint
      if (!url) {
        const token = getToken();
        const headers = token ? { Authorization: `Bearer ${token}` } : {};
        const r = await fetch(`${API_BASE}/documents/${id}/stream`, { headers });
        if (!r.ok) throw new Error('Download failed');
        const blob = await r.blob();
        url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = (docTitle || 'signed-document') + '.pdf';
        a.click();
      ...
      window.open(`${API_BASE}/documents/${id}/stream`, '_blank');
    } finally {
  ...
  if (done) return (
    <div style={s.page}>
      <div style={s.successCard}>
        ...
        <p style={{ color: '#94a3b8', fontSize: '0.78rem', marginBottom: '1.25rem' }}>
        ...
        <button onClick={handleDownload} disabled={downloading} style={{
          ...
          fontWeight: 700, fontSize: '0.95rem', cursor: downloading ? 'wait' : 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          boxShadow: '0 4px 12px rgba(22,163,74,0.3)',
          opacity: downloading ? 0.8 : 1,
        }}>
          {downloading ? '⏳ Downloading...' : '⬇️ Download Signed Document'}
        </button>
        ...
        {effectiveToken && (
          <p style={{ color: '#94a3b8', fontSize: '0.72rem', marginTop: '0.75rem', textAlign: 'center' }}>
  ...
  if (declined) return (
    <div style={s.page}>
      <div style={s.successCard}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '0.5rem' }}>
          <div style={{ width: 72, height: 72, background: '#fee2e2', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' }}>✕</div>
        </div>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0f172a', margin: '0.75rem 0 0.5rem' }}>Decline recorded</h2>
        <p style={{ color: '#64748b', fontSize: '0.95rem', margin: '0 0 0.25rem' }}>You have declined to sign this document.</p>
        {docTitle && <p style={{ color: '#94a3b8', fontSize: '0.82rem', margin: '0 0 1.75rem', fontStyle: 'italic' }}>"{docTitle}"</p>}
        <div style={{ background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 10, padding: '1rem', marginBottom: '1.25rem', textAlign: 'left' }}>
          <p style={{ margin: 0, color: '#78350f', fontSize: '0.875rem', lineHeight: 1.6 }}>
            The sender has been notified and the signing workflow has been stopped. This event has been permanently recorded in the audit trail.
          </p>
        </div>
        {!effectiveToken && (
          <button onClick={() => navigate('/dashboard')} style={{ width: '100%', padding: '0.75rem', background: '#f1f5f9', color: '#374151', border: '1px solid #e2e8f0', borderRadius: 10, fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer' }}>
            Go to Dashboard
          </button>
        )}
        {effectiveToken && <p style={{ color: '#94a3b8', fontSize: '0.72rem', marginTop: '0.75rem', textAlign: 'center' }}>You may now close this window.</p>}
      </div>
      <style>{"@keyframes spin { to { transform: rotate(360deg); } }"}</style>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#475569', display: 'flex', flexDirection: 'column' }}>
        ...
        {useFields ? (
          <>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowDeclineModal(true)} disabled={saving} style={{ padding: '0.7rem 1.1rem', background: 'white', color: '#dc2626', border: '1.5px solid #fca5a5', borderRadius: 8, fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer' }}>✕ Decline</button>
              {!allRequiredFilled && (
                <button onClick={goToNextField}
          ...
          <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
            <button onClick={() => setShowDeclineModal(true)} disabled={saving} style={{ padding: '0.8rem 1.25rem', background: 'white', color: '#dc2626', border: '1.5px solid #fca5a5', borderRadius: 8, fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer' }}>✕ Decline</button>
      ...
      {showDeclineModal && (
        <DeclineModal
          documentName={docTitle || 'this document'}
